...
df_total_time = df['Total seconds']
part1, part2 = st.columns([2, 3])
part1.dataframe(data = df.style.highlight_max(axis=0), height = 250)
part2.bar_chart(data = df_total_time, x_label = "Time Spent Studying", y_label = "Frequency",use_container_width = True)
st.line_chart(data = df_total_time, x_label = "Days", y_label = "Time (in seconds)", use_container_width = True)
...
def log_data():
    dt = total_time - max(0, st.session_state["total_time"])
    ...
    if len(mv) == 1: mv = "0" + mv
    if len(sv) == 1: sv = "0" + sv
    st.write(f'**Elapsed Time:** {hv}:{mv}:{sv}')
